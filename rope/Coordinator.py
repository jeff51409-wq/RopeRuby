...
# @profile
def coordinator():
    global gui, vm, action, frame, r_frame, load_notice, resize_delay, vram_tick, batch_queue, batch_total
    start = time.time()
...
##################    
 ...
 ####################   
    if len(action) > 0:
        ...
        elif action[0][0] == 'start_batch':
            batch_queue = action[0][1][:]
            batch_total = len(action[0][1])
            if batch_queue:
                next_video = batch_queue.pop(0)
                vm.load_target_video(next_video)
                vm.play_video('record')
                done = batch_total - len(batch_queue)
                gui.set_batch_label(f'批量 {done}/{batch_total}')
            action.pop(0)

        elif action[0][0] == 'stop_batch':
            batch_queue = []
            gui.set_batch_label('')
            gui.reset_batch_button()
            action.pop(0)
        ...
        elif action[0][0] == 'progress':
            p = action[0][1]
            eta = p['elapsed'] / p['current'] * (p['total'] - p['current']) if p['current'] > 0 else 0
            gui.set_progress(p['current'], p['total'], eta)
            action.pop(0)
    ...
    if vram_tick >= 60:
        ...
        if torch.cuda.is_available():
            ...
            gui.set_vram(used_gb, total_gb)
        else:
            gui.set_vram(None, None)
...
def run():
    global gui, vm, action, frame, r_frame, resize_delay, batch_queue, batch_total
