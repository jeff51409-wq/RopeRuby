...
class GUI(tk.Tk):
 ...
 ######### Options 
        ...
        # Label Frame 1
        self.label_frame1 = tk.LabelFrame( self.options_frame_canvas1, self.frame_style, height = 71, width = 1550 )
        self.label_frame1.place(x=0, y=0)
        
        column=8
        self.create_ui_button('Upscale', self.label_frame1, column, 8,150)
        self.create_ui_button('Threshold', self.label_frame1, column, 37,150) 

        column=column+125+x_space
        self.create_ui_button('Strength', self.label_frame1, column, 8,150)
        self.create_ui_button('Orientation', self.label_frame1, column, 37,150)        
        
        column=column+125+x_space
        self.create_ui_button('Border', self.label_frame1, column, 8,150)
        self.create_ui_button('Diff', self.label_frame1, column, 37,150)
        
        column=column+125+x_space  
        self.create_ui_button('Occluder', self.label_frame1, column, 8,150)
        self.create_ui_button('FaceParser', self.label_frame1, column, 37,150)        

        column=column+125+x_space  
        self.create_ui_button('CLIP', self.label_frame1, column, 8,150)
        # CLIP-entry
        ...
        column=column+125+x_space
        self.create_ui_button('Blur', self.label_frame1, column, 8,150)   
        self.create_ui_button('MaskView', self.label_frame1, column, 37,150)
        
        column=column+125+x_space
        self.create_ui_button('Color', self.label_frame1, column, 8,150)
        self.create_ui_button('Transform', self.label_frame1, column, 37,150)   

        column=column+125+x_space
        self.create_ui_button('RefDel', self.label_frame1, column, 8,150)
        self.create_ui_button('AutoColor', self.label_frame1, column, 37,150)

        column=column+125+x_space
        self.create_ui_button('FrameEnhance', self.label_frame1, column, 8,150)
 ...
 ######## Target Faces           
        ...
        self.found_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 50), text=" 目标人脸")
 ...
 ######## Source Faces       
        ...
        self.source_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 50), text=' 源人脸')
...
######### Target Videos
        ...
        self.target_button_canvas = tk.Canvas(self.target_videos_frame, self.canvas_style1, height = 130, width = 112)
        ...
        self.create_ui_button_2('BatchRecord', self.target_button_canvas, 8, 66)
        self.create_ui_button_2('StopBatch', self.target_button_canvas, 8, 95)
        ...
        self.target_media_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 50), text=' 目标视频')
 ...
 ######### Options
        ...
        # Info Bar (進度條 + VRAM) — row 7
        self.info_bar_frame = tk.Frame(self, self.frame_style, height=22)
        self.info_bar_frame.grid(row=7, column=0, sticky='NEWS', pady=0)
        self.info_bar_frame.grid_columnconfigure(0, weight=0)
        self.info_bar_frame.grid_columnconfigure(1, weight=0)
        self.info_bar_frame.grid_columnconfigure(2, weight=1)
        self.info_bar_frame.grid_columnconfigure(3, weight=0)
        self.info_bar_frame.grid_remove()

        self.batch_label = tk.Label(self.info_bar_frame, self.label_style, text='', width=8, anchor='w')
        self.batch_label.grid(row=0, column=0, sticky='W', padx=(4, 0))

        self.progress_bar = ttk.Progressbar(self.info_bar_frame, orient='horizontal', mode='determinate', length=120)
        self.progress_bar.grid(row=0, column=1, sticky='W', padx=(4, 0))

        self.progress_label = tk.Label(self.info_bar_frame, self.label_style, text='', anchor='w')
        self.progress_label.grid(row=0, column=2, sticky='W', padx=4)
    ...
    def initialize_gui( self ):
        ...
        try:
            ...
            self.actions['LoadTVideosButton'].configure(self.button_highlight_style, text=' 安装')
        else:
        ...
        try:
            ...
            self.actions['LoadSFacesButton'].configure(self.button_highlight_style, text=' 安装')
        else:
        ...
        try:
            ...
            self.actions['OutputFolderButton'].configure(self.button_highlight_style, text=' 安装')
        else:
        ...
        self.actions['StartRopeButton'].configure(self.button_highlight_style, text=' 加载ROPE')
        ...
        self.set_status('欢迎来到 Rope-Ruby红宝石！')
    ...
    def load_all(self):
        ...
        self.actions['StartRopeButton'].configure(self.inactive_button_style, text=" 加载完毕")
    ...
    def load_source_faces(self):
        if not self.detection_model:
            ...
            faces = []  # 每個元素: [crop_82x82, embedding, tooltip_or_None]

            try:
                entries = sorted(os.listdir(directory))
            except Exception as e:
                print('Cannot list source faces directory:', e)
                entries = []
            ...
            for entry in entries:
                ...
                if os.path.isfile(full_path):
                    # 單張圖片 — 原有邏輯
                    try:
                        file_type = mimetypes.guess_type(full_path)[0][:5]
                    except Exception:
                        print('Unrecognized file type:', full_path)
                        continue
                    if file_type != 'image':
                        continue
                    ...
                    if kpss.shape[0] == 0:
                        continue
                    face_emb, img_out = self.recognize(img, kpss[0])
                    crop = cv2.cvtColor(img_out.cpu().numpy(), cv2.COLOR_BGR2RGB)
                    crop = cv2.resize(crop, (82, 82))
                    faces.append([crop, face_emb, None])

                elif os.path.isdir(full_path):
                    # 子資料夾 — 平均所有人臉 embedding
                    try:
                    ...
                    for sub_entry in sub_entries:
                        ...
                        if not os.path.isfile(sub_path):
                            continue
                        try:
                            file_type = mimetypes.guess_type(sub_path)[0][:5]
                        except Exception:
                            continue
                        if file_type != 'image':
                            continue
                        ...
                        if img is None:
                            continue
                        img_t = torch.from_numpy(img).to('cuda')
                        img_t = img_t.permute(2, 0, 1)
                        kpss = self.detect(img_t, input_size=(640, 640), max_num=1, metric='default')
                        if kpss.shape[0] == 0:
                            print(f'No face in {sub_path}, skipping')
                            continue
                        face_emb, img_out = self.recognize(img_t, kpss[0])
                        embeddings.append(face_emb)
                        if thumbnail is None:
                            crop = cv2.cvtColor(img_out.cpu().numpy(), cv2.COLOR_BGR2RGB)
                            thumbnail = cv2.resize(crop, (82, 82))
                    if not embeddings:
                        print(f'No valid faces in subfolder: {full_path}')
                        continue
                    avg_emb = np.mean(np.array(embeddings), axis=0)
                    norm = np.linalg.norm(avg_emb)
                    if norm > 0:
                        avg_emb = avg_emb / norm
                    tooltip = f'平均 {len(embeddings)} 張 ({entry})'
                    faces.append([thumbnail, avg_emb, tooltip])
            ...
            for i in range(len(faces)):
                ...
                self.source_faces[shift_i]["TKButton"] = tk.Button(
                    self.source_faces_canvas, self.inactive_button_style,
                    image=self.source_faces[shift_i]["Image"], height=86, width=86
                )
                self.source_faces[shift_i]["ButtonState"] = False

                if faces[i][2]:
                    tooltip_text = faces[i][2]
                    self.source_faces[shift_i]["TKButton"].bind(
                        "<Enter>", lambda event, t=tooltip_text: self.set_status(t)
                    )
                    self.source_faces[shift_i]["TKButton"].bind(
                        "<Leave>", lambda event: self.set_status('')
                    )
    ...
    def set_batch_label(self, text):
        self.batch_label.configure(text=text)
        if text:
            self.info_bar_frame.grid()

    def reset_batch_button(self):
        self.actions['BatchRecordButton'].config(self.inactive_button_style)

    def start_batch_record(self):
        if not self.json_dict["source videos"]:
            print("Please select video folder first!")
            return
        if not self.json_dict["saved videos"]:
            print("Set saved video folder first!")
            return

        video_dir = self.json_dict["source videos"]
        video_exts = {'.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv', '.m4v'}
        try:
            all_files = sorted(os.listdir(video_dir))
        except Exception as e:
            print('Cannot list video directory:', e)
            return

        batch_list = [os.path.join(video_dir, f) for f in all_files
        ...
        self.add_action("start_batch", batch_list)
        self.actions['BatchRecordButton'].config(self.active_button_style)
    ...
    def set_vram(self, used_gb, total_gb):
        if used_gb is None:
            self.vram_label.configure(text='CPU 模式', fg='white')
        else:
            pct = used_gb / total_gb if total_gb > 0 else 0
            color = 'red' if pct > 0.9 else 'white'
            self.vram_label.configure(text=f'VRAM {used_gb:.1f}/{total_gb:.1f} GB', fg=color)
        self.info_bar_frame.grid()

    def set_progress(self, current, total, eta_seconds):
        pct = int(current / total * 100) if total > 0 else 0
        self.progress_bar['value'] = pct
        m, s = divmod(int(max(eta_seconds, 0)), 60)
        self.progress_label.configure(text=f'{pct}% | {current}/{total} 幀 | ETA {m}:{s:02d}')
        self.info_bar_frame.grid()
    ...
    def change_video_quality(self, event): 
        ...
        temp = ' '+parameter+' '*(7-len(parameter)-len(temp_num))+temp_num
    ...
    def create_ui_button_2(self, parameter, root, x, y, width=96, height=26):
        ...
        elif parameter == 'BatchRecord':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left', image=self.actions[icon_holder], anchor='w', command=lambda: self.start_batch_record())
        elif parameter == 'StopBatch':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left', image=self.actions[icon_holder], anchor='w', command=lambda: self.add_action("stop_batch", True))
