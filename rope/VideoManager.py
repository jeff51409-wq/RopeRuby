...
KPS_SMOOTH_MAX_FACES = 10     # max tracked faces (matches detect max_num=10)
...
class VideoManager():  
    ...
    def _cache_kps(self, frame_number, kpss):
        with lock:
            if len(self.kps_cache_order) >= 500:
                oldest = self.kps_cache_order.pop(0)
                self.kps_cache.pop(oldest, None)
            self.kps_cache[frame_number] = kpss
            self.kps_cache_order.append(frame_number)
        ...
        if not self.record and time_diff >= 1.0/float(self.fps) and self.play:
            ...
            if index != -1:
                ...
                if self.process_qs[index]['Status'] == 'finished':
                    ...
                    # 進度回報
                    _elapsed = time.time() - self.timer
                    _current = self.process_qs[index]['FrameNumber'] - self.record_start_frame + 1
                    _total = self.video_frame_total - self.record_start_frame
                    if _current > 0 and _total > 0:
                        self.add_action('progress', {'current': _current, 'total': _total, 'elapsed': _elapsed})
    ...
    def swap_video(self, target_image, frame_number, change_parameters):   
        ...
        with lock:
            _cached = self.kps_cache.get(frame_number) if not change_parameters else None
        if _cached is not None:
            kpss = _cached
        else:
            kpss = self.func_w_test("detect", self.detect, img, input_size = (640, 640), max_num=10, metric='default')
            if not change_parameters and kpss is not None:
                self._cache_kps(frame_number, kpss)
    ...
    def _smooth_kps(self, face_emb, raw_kps):
        ...
        with lock:
            if best_idx >= 0 and best_dist < KPS_SMOOTH_MATCH_DIST:
                ...
                self.kps_smooth_state[best_idx]['embedding'] = face_emb
                self.kps_smooth_state[best_idx]['smoothed_kps'] = smoothed
            else:
                smoothed = raw_kps
                if len(self.kps_smooth_state) >= KPS_SMOOTH_MAX_FACES:
                    self.kps_smooth_state.pop(0)
                self.kps_smooth_state.append({'embedding': face_emb, 'smoothed_kps': raw_kps})
